...
def main(argv: list[str] | None = None) -> None:
    # DEPRECATED: convention-aware agent은 이제 논문 Eq.(2)(self-play + BC)를 따르는
    # train_comedi_cbr로 학습한다. 과거 train_mep(best-response) 경로가 실수로 실행되지
    # 않도록, 이 진입점은 CBR로 위임한다. (아래 train_adaptive/_run_stage2는 dead code)
    from comedi_transplant.train_comedi_cbr import main as cbr_main
