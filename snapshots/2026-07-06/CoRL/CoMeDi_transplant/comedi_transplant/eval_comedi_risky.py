...
def _add_comedi_eval_args(parser: argparse.ArgumentParser) -> None:
    ...
    # 1이면 pool 파트너 외에 uniform-random 파트너와의 play도 결과에 포함.
    parser.add_argument("--comedi_eval_vs_random", type=int, default=1)
...
def evaluate_all(all_args) -> None:
    ...
    # 상대가 uniform-random일 때의 play도 결과에 포함(양방향). agent 이름 "random" →
    # evaluate_policy_pair가 RandomEvalPolicy(균등 랜덤 행동)를 주입한다.
    if int(getattr(all_args, "comedi_eval_vs_random", 1) or 0):
        print("CoMeDi final eval: also evaluating vs uniform-random partner")
        _evaluate_one(all_args, adaptive_name, "random", final_eval_dir)
        _evaluate_one(all_args, "random", adaptive_name, final_eval_dir)
