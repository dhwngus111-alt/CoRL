...
BASE_REW_SHAPING_PARAMS = {
    ...
    "POT_DISTANCE_REW": 0,
    "SOUP_DISTANCE_REW": 0,
    # Water-aware PBRS dense shaping: per-step reward for shrinking the (active-water-
    # blocked) walk distance soup->serving / onion->fillable-pot. 0 disables. The
    # shortcut through water only earns shaping once a subgoal has cleared it.
    "DISTANCE_SHAPING_REW": 0.3,
    # Direct reward for pressing a subgoal button that actually clears ACTIVE water
    # (opens a shortcut). Teaches the press itself, which had NO reward before --
    # only re-pressing an already-cleared subgoal earns nothing.
    "SUBGOAL_PRESS_REW": 2,
    "ONION_DROP_REW": 0,
    "SOUP_DROP_REW": 0,
}
...
EVENT_TYPES = [
    ...
    "soup_handoff",
    # Subgoal events
    "subgoal_press",   # agent pressed a button that CLEARED active water (useful press)
    # Potting events
...
class OvercookedGridworld(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Cooperative object-centric distance shaping (rewards hand-offs: an object
        # placed on a counter nearer its goal raises the team potential). Default OFF
        # -> per-agent player-centric shaping (use OFF for adversarial / competitive).
        self.handoff_shaping = bool(kwargs.get("handoff_shaping", False))
        # Competitive finite onion supply: total onions seeded on counters for the
        # whole episode (dispenser disabled). None (default) -> infinite dispenser
        # (cooperative behavior unchanged).
        _cos = kwargs.get("competitive_onion_supply", None)
        self.competitive_onion_supply = None if _cos is None else int(_cos)
    ...
    def _activate_subgoal(self, subgoal_pos):
        """Disable the water linked to this subgoal for subgoal_disable_steps.
        Returns True iff at least one linked water was ACTIVE (this press actually
        opened a shortcut) -- so callers can reward only USEFUL presses, not
        re-pressing an already-cleared subgoal."""
        if subgoal_pos not in self.subgoal_to_water:
            return False

        cleared_active = False
        for water_pos in self.subgoal_to_water[subgoal_pos]:
            if self.water_disable_timers.get(water_pos, 0) <= 0:
                cleared_active = True   # water was active -> this press clears it
    ...
    # ------------------------------------------------------------------ #
    # Water-aware distance shaping (dense reward)                         #
    # ------------------------------------------------------------------ #
    def _distance_shaping_goals(self, state, player):
        """Feature cells the carried object should head toward: onion -> any pot that
        can still take an onion; dish -> a pot with a cooking/ready soup; soup ->
        serving (DIRECT carry only -- soup is never hand-off-shaped, see
        distance_shaping_potentials). Returns (x,y) positions, or None."""
        held = player.held_object
        if held is None:
            # Empty-handed: head to the dispenser for the object the team next needs.
            # Adds dense guidance for the ACQUISITION leg (empty -> dispenser), which
            # previously had NO shaping (a cause of the onion-stage stall).
            return self._acquisition_goal(state) or None
        if held.name == 'onion':
            return self._fillable_pots(state) or None
        if held.name == 'dish':
            return self._dish_goal_pots(state) or None
        if held.name == 'soup':
            return self.get_serving_locations()
        return None

    def _acquisition_need(self, state):
        """What the team should FETCH next + that object's CARRY goal:
        a cooking/ready soup with NO dish in play -> ('dish', dish-goal pots); else a
        pot that can still take onions -> ('onion', fillable pots); else (None, None).
        The carry goal lets the acquisition potential count fetch + carry cost-to-go
        so picking the object up is smooth (no Phi drop)."""
        dish_in_play = len(state.player_objects_by_type.get('dish', [])) > 0
        dish_pots = self._dish_goal_pots(state)
        if dish_pots and not dish_in_play:
            return 'dish', dish_pots
        fillable = self._fillable_pots(state)
        if fillable:
            return 'onion', fillable
        return None, None

    def _acquisition_sources(self, state, need):
        """All places an empty agent can ACQUIRE `need` from: the dispensers PLUS any
        reachable counter already holding that object (so the agent grabs a nearby
        counter onion instead of walking to the dispenser)."""
        disp = (self.get_dish_dispenser_locations() if need == 'dish'
                else self.get_onion_dispenser_locations())
        counters = [c for c in self.reachable_counters
                    if state.has_object(c) and state.get_object(c).name == need]
        return list(disp), counters

    def _acquisition_goal(self, state):
        """EMPTY-HANDED target = the nearest SOURCE of the object the team next needs
        (dispenser OR a counter already holding it). Player-centric shaping heads the
        empty agent to whichever is closest. State-determined so a useless source is
        not rewarded; once one agent grabs a dish the other re-targets onions."""
        need, _ = self._acquisition_need(state)
        if need is None:
            return None
        disp, counters = self._acquisition_sources(state, need)
        srcs = disp + counters
        return srcs or None

    def _fillable_pots(self, state):
        """Pots that can still take another onion (empty, or partially full and
        not cooking/ready/full)."""
        fillable = []
        for pot_pos in self.get_pot_locations():
            if not state.has_object(pot_pos):
                fillable.append(pot_pos)
            else:
                soup = state.get_object(pot_pos)
                if (not soup.is_cooking) and (not soup.is_ready) and (not soup.is_full):
                    fillable.append(pot_pos)
        return fillable

    def _dish_goal_pots(self, state):
        """Pots a carried DISH should head to: those holding a soup that is cooking
        or ready (will yield a soup to pick up). Empty if no pot is producing one."""
        pots = []
        for pot_pos in self.get_pot_locations():
    ...
    def _safe_walk_distance(self, start, goal_locs, block_water=True):
        """BFS step-distance from `start` to the nearest cell ORTHOGONALLY ADJACENT
        to any goal in `goal_locs`. floor (' ') and 'G' are traversable. If
        `block_water` (default), ACTIVE water 'W' is BLOCKED -- the risky shortcut is
        gated behind a subgoal clearing it -- which is correct for an agent CARRYING an
        object (a slip would drop it). For an EMPTY agent there is nothing to drop, so
        water is harmless: pass `block_water=False` to let it cut straight through.
        `start` is always a valid source even if currently on water. int steps or None."""
        if not goal_locs:
            return None
        goal_set = set(tuple(g) for g in goal_locs)
        ...
        while q:
            ...
            for dx, dy in adj:
                ...
                if (nx, ny) in visited:
                    continue
                # Walkable = ' ' (floor + cleared water) and 'G' (subgoal button);
                # ACTIVE water 'W' is blocked only when block_water (carrying agent).
                walkable = (' ', 'G') if block_water else (' ', 'G', 'W')
                if self.terrain_mtx[ny][nx] not in walkable:
                    continue
                visited.add((nx, ny))
                q.append(((nx, ny), d + 1))
        return None

    def _objectcentric_distance_total(self, state):
        """Total water-aware distance of every IN-TRANSIT object to its goal: onions
        -> nearest fillable pot; dishes -> a pot with a cooking/ready soup; soups ->
        nearest serving. Objects live at a holder's position OR on a counter (dispenser
        supply excluded). Used by the cooperative hand-off shaping: moving an object
        closer to its goal -- INCLUDING by placing it on a counter nearer the goal --
        lowers this total (raises Phi = -total), so a useful hand-off is rewarded."""
        fillable = self._fillable_pots(state)
        dish_pots = self._dish_goal_pots(state)
        serve = self.get_serving_locations()
        onion_locs, dish_locs, soup_locs = [], [], []
        for player in state.players:
            held = player.held_object
            if held is None:
                continue
            if held.name == 'onion':
                onion_locs.append(player.position)
            elif held.name == 'dish':
                dish_locs.append(player.position)
            elif held.name == 'soup':
                soup_locs.append(player.position)
        for counter in self.reachable_counters:
            if not state.has_object(counter):
                continue
            obj = state.get_object(counter)
            if obj.name == 'onion':
                onion_locs.append(counter)
            elif obj.name == 'dish':
                dish_locs.append(counter)
            elif obj.name == 'soup':
                soup_locs.append(counter)
        # Penalty distance for a goal that is UNREACHABLE without crossing active
        # water: larger than any real path (<= cell count) so an unreachable object
        # is the WORST potential (never lures the agent into a water-gated pocket),
        # and clearing the water via a subgoal press RAISES Phi -> rewards the press
        # (the old "unreachable -> 0" made opening a far path score as NEGATIVE).
        unreach = self._unreachable_penalty()
        total = 0.0
        for locs, goals in ((onion_locs, fillable), (dish_locs, dish_pots),
                            (soup_locs, serve)):
            if not goals:
                continue
            for loc in locs:
                d = self._safe_walk_distance(loc, goals)
                total += d if d is not None else unreach
        # ACQUISITION leg (team cost-to-go): the team must FETCH the next needed object
        # and CARRY it to its goal. Add the MIN (over empty agents x sources) of
        # fetch + carry, so (1) a nearby COUNTER object is preferred over a far
        # dispenser, and (2) the carry leg is pre-counted -> picking the object up is
        # SMOOTH (no Phi drop the moment it's grabbed). For a COUNTER source the carry
        # (counter->goal) is ALREADY in `total` above, so only its fetch leg is added
        # (no double count); a DISPENSER spawns a NEW object so both legs are added.
        need, carry_goals = self._acquisition_need(state)
        if need:
            disp, counter_srcs = self._acquisition_sources(state, need)
            # carry leg is agent-independent -> compute once per source
            def _carry(s):
                d = self._safe_walk_distance(s, carry_goals)
                return unreach if d is None else d
            disp_carry = {s: _carry(s) for s in disp}
            cnt_carry = {s: _carry(s) for s in counter_srcs}
            # Choose the (empty agent, source) with the MIN full cost-to-go = fetch +
            # carry -- a FAIR "nearest" comparison over ALL sources, NOT a hardwired
            # counter preference (a counter far from the pot must NOT beat a good
            # dispenser). Then add full - already_counted: a COUNTER's carry is already
            # in `total` (in-transit) so only its fetch is added; a DISPENSER spawns a
            # NEW object so its full fetch+carry is added.
            cands = []  # (full_cost_to_go, carry_already_counted_in_total)
            for p in state.players:
                if p.held_object is not None:
                    continue
                for s in disp:
                    f = self._safe_walk_distance(p.position, [s], block_water=False)  # empty fetch: water harmless
                    cands.append(((unreach if f is None else f) + disp_carry[s], 0.0))
                for s in counter_srcs:
                    f = self._safe_walk_distance(p.position, [s], block_water=False)  # empty fetch: water harmless
                    cands.append(((unreach if f is None else f) + cnt_carry[s],
                                  cnt_carry[s]))
            if cands:
                full, counted = min(cands, key=lambda x: x[0])
                total += full - counted
        return total

    def _unreachable_penalty(self):
        """Distance for a goal UNREACHABLE without crossing active water: must exceed
        any real reachable path (so unreachable = WORST potential, no lure) yet stay
        BOUNDED so its F spike can't dominate. # of standable cells is a tight upper
        bound on any shortest path (was W*H, ~2x looser). Cached (terrain is static)."""
        p = getattr(self, '_unreach_pen_cache', None)
        if p is None:
            p = float(len(self.get_valid_player_positions()))
            self._unreach_pen_cache = p
        return p

    def distance_shaping_potentials(self, state):
        """Per-agent PBRS potential Phi(s), evaluated against the CURRENT terrain (so
        callers get the PRE-press value before resolve_interacts and the POST-press
        value after).

        - handoff_shaping (cooperative): ONE object-centric TEAM potential =
          -(total distance of all in-transit objects to their goals), returned for
          BOTH agents. Placing an object on a counter nearer its goal (a useful
          hand-off) raises Phi -> reward; a useless drop lowers it.
        - else (default / adversarial): per-agent player-centric = -(distance from
          each agent to ITS OWN carried object's goal)."""
        if getattr(self, 'handoff_shaping', False):
            # Object-centric (hand-off-aware) for ALL in-transit objects (onion, dish,
            # soup): ONE team potential = -(total distance), returned for both agents.
            # Placing any object on a counter nearer its goal raises Phi -> reward.
            phi = -float(self._objectcentric_distance_total(state))
            return [phi for _ in state.players]
        # Unreachable-without-water goal -> WORST potential (penalty > any real path),
        # not 0, so it never lures and a subgoal press that opens the path raises Phi.
        # A goal that does NOT EXIST (empty-handed with nothing to fetch) stays 0.
        unreach = self._unreachable_penalty()
        potentials = []
        for player in state.players:
            goals = self._distance_shaping_goals(state, player)
            if not goals:
                potentials.append(0.0)
                continue
            # EMPTY agent (heading to a dispenser to fetch) has nothing to drop -> water
            # is harmless -> pass straight through; a CARRYING agent blocks water (risk).
            d = self._safe_walk_distance(player.position, goals,
                                         block_water=player.has_object())
            potentials.append(-unreach if d is None else -float(d))
        return potentials

    def calculate_distance_based_shaped_reward(self, phi_old, new_state, gamma=0.95):
        """UNDISCOUNTED progress shaping: per agent,
            F = DISTANCE_SHAPING_REW * (Phi(s') - Phi(s)).
        Rewards NET PROGRESS on the cost-to-go (closer +, away -, STAY 0). Unlike
        discounted PBRS (gamma*Phi'-Phi), there is NO (1-gamma)*Phi 'survival drift',
        so the per-episode shaped reward stays bounded (telescopes to scale*(Phi_T-
        Phi_0) ~ scale*|Phi_0|) and never cancels the per-step time_cost. Not strictly
        policy-invariant for a gamma<1 learner, but the only induced bias is 'make
        progress toward the goal on Phi', which is exactly the intent (Phi is a proper
        cost-to-go after the unreachable-penalty/acquisition fixes). `gamma` kept for
        signature compatibility (UNUSED). `phi_old` is captured BEFORE resolve_interacts
        so a subgoal press registers as a Phi RISE. DISTANCE_SHAPING_REW (0 -> off)."""
        scale = self.reward_shaping_params.get("DISTANCE_SHAPING_REW", 0.0)
        if not scale:
            return [0.0 for _ in new_state.players]
        phi_new = self.distance_shaping_potentials(new_state)
        return [scale * (pn - po) for po, pn in zip(phi_old, phi_new)]
    ...
    def get_state_transition(
        ...
        # Capture the distance-shaping potential BEFORE resolve_interacts: a subgoal
        # press inside resolve_interacts clears water (terrain changes), so phi_old
        # must see the PRE-press terrain for that press to register as progress.
        phi_old = self.distance_shaping_potentials(state)
        ...
        # Re-activate expired subgoal water BEFORE computing phi_new, so a water
        # RE-activation's Phi DROP is charged into THIS step's F (PBRS telescoping:
        # phi_new == next step's phi_old). If ticked AFTER, the drop falls between
        # steps uncharged and the press's positive F is never compensated -> the
        # agent can farm shaping by re-pressing each disable cycle (amplified by the
        # unreachable -(W*H) penalty). A press THIS step keeps a fresh timer (60->59),
        # so phi_new still sees its water cleared and the press still registers as +F.
        self._tick_subgoal_timers()

        # Dense reward: water-aware PBRS distance shaping. phi_old was captured
        # BEFORE resolve_interacts so a subgoal press this step raises the potential.
        dist_shaped = self.calculate_distance_based_shaped_reward(
            phi_old, new_state, gamma=getattr(self, 'shaping_gamma', 0.95))
        shaped_reward_by_agent = [r + dr for r, dr in zip(shaped_reward_by_agent, dist_shaped)]
        infos = {
    ...
    def resolve_interacts(self, new_state, joint_action, events_infos):
        ...
        for player_idx, (player, action) in enumerate(
            ...
            if current_terrain_type == "G":
                if self._activate_subgoal(pos):
                    # Reward only a USEFUL press (one that actually cleared active
                    # water -> opened a shortcut). Pressing had NO reward before.
                    shaped_reward[player_idx] += self.reward_shaping_params["SUBGOAL_PRESS_REW"]
                    events_infos["subgoal_press"][player_idx] = True  # logged -> wandb count
                continue
            ...
            elif terrain_type == "O" and player.held_object is None and self.competitive_onion_supply is not None:
                # Competitive finite-supply: onion DISPENSER disabled (the only onions
                # are those seeded on counters at episode start by seed_competitive_onions).
                # No-op branch -> the original dispenser branch below is untouched.
                pass
...
# 물건을 들고 물로 진입했을 때만 미끄러질 수 있음
    ...
    def seed_competitive_onions(self, state, n_onions=None, rng=None):
        """Place `n_onions` onion ObjectStates on random FREE reachable counters.

        Competitive finite-supply mode (competitive_onion_supply): the onion
        dispenser is disabled, so these seeded onions are the episode's WHOLE onion
        supply. Counter onions are visible in the lossless obs. Mutates + returns
        `state`; no-op if the supply is unset. Counters already holding an object are
        skipped; seeds as many as fit if fewer free counters than requested exist.
        """
        if n_onions is None:
            n_onions = self.competitive_onion_supply
        if n_onions is None or n_onions <= 0:
            return state
        rng = np.random if rng is None else rng
        candidate_counters = list(self.reachable_counters) or self.get_counter_locations()
        free_counters = [c for c in candidate_counters if not state.has_object(c)]
        if not free_counters:
            return state
        n = int(min(n_onions, len(free_counters)))
        chosen_idx = rng.choice(len(free_counters), size=n, replace=False)
        for idx in np.atleast_1d(chosen_idx):
            pos = free_counters[int(idx)]
    ...
    # one-step ahead #
    ###################
    def one_step_lookahead(self, state, joint_action, encoded=True, as_tensor=False, device=None, obs_encoder="vector", horizon=400, feasible_fn=None):
        """
        ...
        def make_outcome(ja, _state, p):
            # [joint_action, obs, prob] (+ optional joint-feasibility mask for THIS
            # next-state, so the target can exclude infeasible joints -> Bug3 fix).
            o = [ja, make_obs(_state), p]
            if feasible_fn is not None:
                o.append(feasible_fn(_state))
            return o
        ...
        if np.sum(can_slip) == 0:
            prob = 1
            outcomes.append(make_outcome(joint_action, next_state, prob))
