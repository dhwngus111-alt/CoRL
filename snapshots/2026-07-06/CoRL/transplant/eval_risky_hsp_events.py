...
def _default_run_id(all_args) -> str:
    run_name = str(getattr(all_args, "wandb_run_name", "") or _default_run_name(all_args))
    run_id = re.sub(r"[^A-Za-z0-9_.-]+", "_", run_name).strip("._-")
    return run_id or f"05_hsp_s1_event_eval_{all_args.layout_name}_seeds1-{all_args.seed_max}"
...
def main(args=None):
    ...
    base_args.wandb_run_id = str(getattr(base_args, "wandb_run_id", "") or "").strip() or _default_run_id(base_args)
    ...
    try:
        for seed in range(1, int(base_args.seed_max) + 1):
            ...
            for direction_index, (agent0, agent1, direction) in enumerate(_directions(seed)):
                ...
                pair_args.score_table_output_path = None
                pair_args.gif_manifest_output_path = None
