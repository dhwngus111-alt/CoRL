...
printf '\n\n\n=== [05] HSP S1 이벤트 평가 시작 ===\n\n\n'
echo "event eval settings: EVAL_EPISODES=${EVAL_EPISODES}, N_EVAL_ROLLOUT_THREADS=${N_EVAL_ROLLOUT_THREADS}, TIME_COST=${TIME_COST}"
...
EVENT_EVAL_WANDB_RUN_NAME="${EVENT_EVAL_WANDB_RUN_NAME:-05_hsp_s1_event_eval_${WANDB_RUN_PREFIX}_seeds1-${SEED_MAX}}"
EVENT_EVAL_WANDB_RUN_ID="${EVENT_EVAL_WANDB_RUN_ID:-${EVENT_EVAL_WANDB_RUN_NAME}}"
echo "event eval W&B run: ${EVENT_EVAL_WANDB_RUN_NAME} (id=${EVENT_EVAL_WANDB_RUN_ID})"

"${PYTHON}" -m transplant.eval_risky_hsp_events \
  "${COMMON_ARGS[@]}" \
  --algorithm_name population \
  --experiment_name hsp-S1-event-eval \
  --wandb_stage_name 05_hsp_s1_event_eval \
  --wandb_bundle_run_name "${EVENT_EVAL_WANDB_RUN_NAME}" \
  --wandb_run_id "${EVENT_EVAL_WANDB_RUN_ID}" \
  --seed_max "${SEED_MAX}" \
