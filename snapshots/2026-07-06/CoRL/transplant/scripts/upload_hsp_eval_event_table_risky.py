...
def _make_gif_panel_payload(gif_rows: list[dict], wandb, panel_section: str = "final_eval_gifs") -> dict:
    ...
    for row in gif_rows:
        ...
        if panel_name is None:
            continue
        # 스펙: partner_group별 섹션(policy_pool_gif / random_gif). group 없으면 설정값.
...
def _upload_to_wandb(
    ...
    # 스펙: partner_group(policy_pool / random)별로 {group}_eval 섹션 생성.
    #   {group}_eval/mean_event_by_partner (Table), overall/agent0/agent1_average (scalar),
    #   mean_event_heatmap (Image).
    for group in ("policy_pool", "random"):
        g_rows = [r for r in rows if str(r.get("partner_group", "")) == group]
        g_scores = [r for r in score_rows if str(r.get("partner_group", "")) == group]
        if not g_rows and not g_scores:
            continue
        payload = {}
        if g_rows:
            g_matrices = _write_matrices(g_rows, args.output_dir, f"{args.adaptive_policy_name}_{group}")
            mean_path = g_matrices.get("mean")
            if mean_path is not None and Path(mean_path).exists():
                columns, data = _read_matrix(mean_path)
                payload[f"{group}_eval/mean_event_by_partner"] = wandb.Table(columns=columns, data=data)
                g_heatmap = _write_heatmap(
                    mean_path,
                    args.output_dir / f"{args.adaptive_policy_name}_{group}_event_counts_heatmap_mean.png",
                    f"{args.adaptive_policy_name} {group} eval event counts",
                )
                if g_heatmap is not None:
                    payload[f"{group}_eval/mean_event_heatmap"] = wandb.Image(str(g_heatmap))
        for label, agent_filter in (
            ("overall_average", None),
            ("agent0_average", "agent0"),
            ("agent1_average", "agent1"),
        ):
            value = _score_mean(g_scores, agent_filter)
            if value is not None:
                payload[f"{group}_eval/{label}"] = value
        if payload:
            run.log(payload)

    # 스펙: GIF도 partner_group별 섹션(policy_pool_gif / random_gif) — _make_gif_panel_payload가 처리.
