...
# 여기서는 경로, Python 실행기, risky 환경 인자, PPO/HSP/MEP 기본값을 한곳에서 관리한다.

# 프로젝트 루트 경로들.
# TRANSPLANT_ROOT: 이번 transplant 실험 코드와 결과가 놓이는 루트.
# CORL_ROOT: CoRL 작업 디렉터리 루트.
# HSP_ROOT/RISKY_ROOT: 원본 HSP 코드와 Risky Overcooked 코드 위치.
...
export RISKY_ROOT="${RISKY_ROOT:-${CORL_ROOT}/risky}"
export POLICY_POOL="${POLICY_POOL:-${TRANSPLANT_ROOT}/policy_pool}"

# transplant, HSP, risky/src를 import할 수 있도록 Python path를 묶는다.
export PYTHONPATH="${CORL_ROOT}:${HSP_ROOT}:${RISKY_ROOT}/src:${PYTHONPATH:-}"
...
# Risky Overcooked 환경 기본값.
# LAYOUT은 risky_multipath로 고정해서 Stage 1/2와 평가가 같은 layout을 보게 한다.
# EPISODE_LENGTH=200은 새 Risky transplant 실험 기본 horizon이다.
# SUBGOAL_DISABLE_STEPS는 G subgoal interact 후 puddle이 비활성화되는 step 수이다.
# P_SLIP/TIME_COST는 Risky 환경에만 있는 확률적 slip과 step penalty 계열 설정이다.
LAYOUT="${LAYOUT:-risky_multipath}"
NUM_AGENTS="${NUM_AGENTS:-2}"
EPISODE_LENGTH="${EPISODE_LENGTH:-200}"
SUBGOAL_DISABLE_STEPS="${SUBGOAL_DISABLE_STEPS:-60}"
P_SLIP="${P_SLIP:-0.15}"
TIME_COST="${TIME_COST:-0.0}"
...
# Stage 1 full run에서는 보통 NUM_ENV_STEPS=10000000, N_ROLLOUT_THREADS=100을 쓴다.
N_ROLLOUT_THREADS="${N_ROLLOUT_THREADS:-4}"
N_EVAL_ROLLOUT_THREADS="${N_EVAL_ROLLOUT_THREADS:-1}"
NUM_ENV_STEPS="${NUM_ENV_STEPS:-120000}"
...
# W&B entity 이름. 프로젝트 이름은 Python runner 쪽에서 env_name 등을 기준으로 잡힌다.
