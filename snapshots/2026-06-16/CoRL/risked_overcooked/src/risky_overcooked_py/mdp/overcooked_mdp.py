...
class OvercookedGridworld(object):
    ...
    def _acquisition_goal(self, state):
        """EMPTY-HANDED target = the dispenser for the object the team next needs:
        a cooking/ready soup with NO dish in play -> fetch a DISH; else a pot that
        can still take onions -> fetch an ONION; else None (nothing to acquire).
        State-determined so going to a USELESS dispenser is not rewarded, and once
        one agent grabs a dish the other re-targets onions (no double dish-fetch)."""
        dish_in_play = len(state.player_objects_by_type.get('dish', [])) > 0
        if self._dish_goal_pots(state) and not dish_in_play:
            return self.get_dish_dispenser_locations()
        if self._fillable_pots(state):
            return self.get_onion_dispenser_locations()
        return None
    ...
    def _safe_walk_distance(self, start, goal_locs):
        """BFS step-distance from `start` to the nearest cell ORTHOGONALLY ADJACENT
        to any goal in `goal_locs`. Only floor (' ') is traversable; ACTIVE water
        ('W') is blocked, so the risky shortcut is invisible until a subgoal clears
        it (the cell flips to ' '). `start` is always a valid source even if it is
        currently on water. Returns int steps, or None if unreachable."""
        if not goal_locs:
            return None
        ...
        while q:
            ...
            for dx, dy in adj:
                ...
                # ACTIVE water 'W' stays blocked so the risky shortcut is gated.
                if self.terrain_mtx[ny][nx] not in (' ', 'G'):
                    continue
                visited.add((nx, ny))
                q.append(((nx, ny), d + 1))
        return None
    ...
    def _objectcentric_distance_total(self, state):
        ...
        for locs, goals in ((onion_locs, fillable), (dish_locs, dish_pots),
            ...
            for loc in locs:
                ...
                if d is not None:
                    total += d
        # ACQUISITION leg (team term): the team must also FETCH the next object it
        # needs. Count the CLOSEST empty-handed agent's distance to that dispenser
        # (min, so it is not double-counted across both empty agents). An empty agent
        # walking toward the needed dispenser lowers the total -> raises Phi -> reward.
        acq_goals = self._acquisition_goal(state)
        if acq_goals:
            empty_dists = [self._safe_walk_distance(p.position, acq_goals)
                           for p in state.players if p.held_object is None]
            empty_dists = [d for d in empty_dists if d is not None]
            if empty_dists:
                total += min(empty_dists)
        return total
    ...
    def distance_shaping_potentials(self, state):
        ...
        for player in state.players:
            ...
            potentials.append(0.0 if d is None else -float(d))
        return potentials

    def calculate_distance_based_shaped_reward(self, phi_old, new_state, gamma=0.99):
        """Potential-based dense shaping (Ng et al. 1999): per agent,
            F = DISTANCE_SHAPING_REW * (gamma * Phi(s') - Phi(s)).
        `phi_old` is the per-agent potential captured BEFORE resolve_interacts (so a
        subgoal press, which opens a shortcut, RAISES Phi(s') and is rewarded this
        step). `gamma` should match the learner's discount for exact policy-
        invariance (default 0.99). Scaled by DISTANCE_SHAPING_REW (0 -> disabled)."""
        ...
        return [scale * (gamma * pn - po) for po, pn in zip(phi_old, phi_new)]
    ...
    def get_state_transition(
        ...
        dist_shaped = self.calculate_distance_based_shaped_reward(phi_old, new_state)
