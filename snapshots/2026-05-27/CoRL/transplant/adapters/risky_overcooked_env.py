...
from risky_overcooked_py.mdp.overcooked_mdp import (  # noqa: E402
    BASE_REW_SHAPING_PARAMS,  # risky/src/risky_overcooked_py/mdp/overcooked_mdp.py 에서 reward shaping을 가져온다.
    OvercookedGridworld,
)
...
class RiskyOvercooked(gym.Env):
    ...
    def __init__(
        ...
        self.layout_name = getattr(all_args, "layout_name", "risky_multipath")
        if self.layout_name != "risky_multipath":
            raise ValueError(
                "Risky transplant v1 is pinned to layout_name='risky_multipath'. "
                f"Got {self.layout_name!r}."
            )

        self.episode_length = int(getattr(all_args, "episode_length", 400))
        ...
        self.time_cost = float(getattr(all_args, "time_cost", 0.0))
    ...
    def _render_frame(self, state=None, grid=None, hud_data=None) -> np.ndarray:
    ...
    def _record_render_step(self, joint_action, sparse_reward: float, done: bool, info: dict) -> None:
        ...
        self.render_traj["infos"].append(dict(info))
    ...
    def _save_render_gif(self, info: dict) -> Path | None:
        ...
        frames = [
            self._render_frame(state=state, grid=grid, hud_data=hud_data)
            for state, grid, hud_data in zip(
    ...
    def _make_mdp(self, all_args) -> OvercookedGridworld:
        ...
        mdp_kwargs = {
            ...
            "p_slip": float(getattr(all_args, "p_slip", 0.15)),
            "neglect_boarders": bool(getattr(all_args, "neglect_boarders", False)),
        }
        if hasattr(all_args, "subgoal_disable_steps"):
            mdp_kwargs["subgoal_disable_steps"] = int(all_args.subgoal_disable_steps)
    ...
    def step(self, action):
        ...
        step_shaped_info = risky_events_to_category_info(mdp_info.get("event_infos", {}))
        ...
        info["risky_event_infos"] = mdp_info.get("event_infos", {})
        info["risky_event_counts"] = self._risk_event_counts(mdp_info.get("event_infos", {}))
        ...
        if self.use_phi:
            ...
            if self.agent_idx == 0:
                hidden_reward = (
                    float(
                        np.dot(self.w0[:-1], vec_hidden[0])
                        + task_sparse_reward * self.w0[-1]
                        + self.time_cost
                    ),
                    float(
                        np.dot(self.w1[:-1], vec_hidden[1])
                        + task_sparse_reward * self.w1[-1]
                        + self.time_cost
                    ),
                )
            else:
                hidden_reward = (
                    float(
                        np.dot(self.w1[:-1], vec_hidden[0])
                        + task_sparse_reward * self.w1[-1]
                        + self.time_cost
                    ),
                    float(
                        np.dot(self.w0[:-1], vec_hidden[1])
                        + task_sparse_reward * self.w0[-1]
                        + self.time_cost
                    ),
                )
