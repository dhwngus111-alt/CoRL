...
STAGE2_TRAIN_FIELDS = ("step", *STAGE2_TRAIN_METRICS)
...
class RiskyOvercookedRunner(HSPSharedRunner):
    ...
    @staticmethod
    def _direct_adaptive_pair(pair: str):
        if pair.startswith(("agent0-", "agent1-", "either-")):
            return None
        left_prefix = "hsp_adaptive-"
        right_suffix = "-hsp_adaptive"
        if pair.startswith(left_prefix):
            partner = pair[len(left_prefix) :]
            if partner:
                return "hsp_adaptive", partner, 0, partner
        if pair.endswith(right_suffix):
            partner = pair[: -len(right_suffix)]
            if partner:
                return partner, "hsp_adaptive", 1, partner
        return None
    ...
    def _stage2_train_summary_row(self, train_infos: dict, total_num_steps: int) -> dict:
        ...
        for metric in STAGE2_TRAIN_METRICS:
            value = train_infos.get(f"hsp_adaptive-{metric}", train_infos.get(metric))
    ...
    def log_train(self, train_infos, total_num_steps):
        ...
        if section == "stage2" and not self._stage2_log_raw_scalars:
            ...
            scalars = {
                f"stage2/train/hsp_adaptive/{metric}": row[metric]
                for metric in STAGE2_TRAIN_METRICS
                if row[metric] != ""
            }
    ...
    def log_env(self, env_infos, total_num_steps):
        ...
        if section == "stage2" and not self._stage2_log_raw_scalars:
            ...
            if self.use_wandb:
                payload = self._stage2_table_payload(sampling_rows)
                if payload:
