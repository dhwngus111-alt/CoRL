...
# 얘로 greedy selection 진행
def select_policies(runs, metric_np, K):
    S = []
    n = len(runs)

    # 처음 1시드는 랜덤으로, 전체 후보군 중에서 집합 S에 추가
    S.append(np.random.randint(0, n))
    
    # 목표 개수 K개가 채워질 때까지 반복
    for iter in range(1, K):
        v = np.zeros((n,), dtype=np.float32)
        for i in range(n):
            if i not in S:
                
                # 이미 선택된 정책 집합 내의 모든 정책 j들과의 Manhattan 거리 차이의 누적합 계산함.
                for j in S:
                    v[i] += abs(metric_np[i] - metric_np[j]).sum()
            else:
                v[i] = -1e9
        
        # 집합 S에 들어오지 않은 후보군 policy i들 중에서 집합군 S에 포함된 policy 전체와 가장 떨어진 행동을 하는 policy를 추가함.
        x = v.argmax()
        S.append(x)
...
if __name__ == "__main__":
    ...
    try:
        ...
        csv_path = f"{eval_result_dir}/event_count.csv"
        print(f"Excel export unavailable ({exc}); writing {csv_path} instead.")
        df.to_csv(csv_path)
