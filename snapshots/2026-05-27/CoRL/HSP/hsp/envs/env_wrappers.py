...
from hsp.utils.util import tile_images
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            if data == "rgb_array":
                ...
                env.render(mode=data)
        elif cmd == 'get_last_render_gif_path':
            paths = getattr(env, "render_gif_paths", None)
            if paths is None:
                path = getattr(env, "last_render_gif_path", None)
                remote.send([] if path is None else [str(path)])
            else:
                remote.send([str(path) for path in paths])
        elif cmd == 'get_spaces':
            remote.send(
                (env.observation_space, env.share_observation_space, env.action_space))
        elif cmd == 'get_max_step':
...
class ChooseSubprocVecEnv(ShareVecEnv):
    ...
    def get_last_render_gif_paths(self):
        for remote in self.remotes:
            remote.send(('get_last_render_gif_path', None))
...
class DummyVecEnv(ShareVecEnv):
    ...
    def get_last_render_gif_paths(self):
        ...
        for env in self.envs:
            env_paths = getattr(env, "render_gif_paths", None)
            if env_paths is None:
                path = getattr(env, "last_render_gif_path", None)
                env_paths = [] if path is None else [path]
            paths.append([str(path) for path in env_paths])
        return paths
