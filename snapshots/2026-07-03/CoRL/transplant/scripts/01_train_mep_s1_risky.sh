...
# 실행 경로, Python 경로, 결과 저장 경로, risked 환경 공통 인자를 불러온다.
...
# 출력 예시: transplant/policy_pool/risky_dualpath_subgoal/mep/s1/train.yml
...
"${PYTHON}" -m transplant.train_risky_adaptive \
  ...
  --wandb_stage_name 01_mep_s1 \
  --wandb_run_name "01_mep_s1_${WANDB_RUN_PREFIX}_seed${SEED:-1}" \
  --seed "${SEED:-1}" \
  --stage 1 \
