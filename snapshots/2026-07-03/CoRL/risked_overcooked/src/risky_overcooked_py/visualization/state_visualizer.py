...
class StateVisualizer:
    ...
    def _render_grid(self, surface, grid, mdp=None):
        ...
        water_disable_timers = (
            getattr(mdp, "water_disable_timers", {}) or {}
            if mdp is not None
            else {}
        )
        disabled_water_positions = {
            water_pos
            for water_pos, timer in water_disable_timers.items()
            if timer > 0
        }
        for y_tile, row in enumerate(grid):
            for x_tile, tile in enumerate(row):
                frame_tile = (
                    EMPTY
                    if tile == WATER and (x_tile, y_tile) in disabled_water_positions
                    else tile
                )
                self.TERRAINS_IMG.blit_on_surface(
                    surface,
                    self._position_in_unscaled_pixels((x_tile, y_tile)),
                    StateVisualizer.TILE_TO_FRAME_NAME[frame_tile],
                )
                if tile == SUBGOAL:
        ...
        # Disabled linked water is drawn as a blue checkpoint marker. Active
        # water keeps the normal water sprite with no overlay.
        if mdp is not None and getattr(mdp, "subgoal_to_water", None):
            ...
            disabled_marker_color = (40, 110, 230)
            disabled_marker_edge = (110, 175, 255)
            disabled_marker_check = (245, 250, 255)

            for wx, wy in sorted(water_positions, key=lambda p: (p[1], p[0])):
                if water_disable_timers.get((wx, wy), 0) <= 0:
                    continue
                ...
                r = marker_radius + max(1, self.UNSCALED_TILE_SIZE // 12)
                diamond = [
                    (center_x, center_y - r),
                    (center_x + r, center_y),
                    (center_x, center_y + r),
                ...
                pygame.draw.polygon(surface, disabled_marker_color, diamond)
                pygame.draw.polygon(surface, disabled_marker_edge, diamond, width=2)
                check_points = [
                    (center_x - r // 2, center_y),
                    (center_x - r // 6, center_y + r // 3),
                    (center_x + r // 2, center_y - r // 3),
                ]
                pygame.draw.lines(
                    surface,
                    disabled_marker_check,
                    False,
                    check_points,
                    width=max(1, r // 4),
                )
