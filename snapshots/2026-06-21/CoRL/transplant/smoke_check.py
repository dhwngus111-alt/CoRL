...
def parse_args(argv):
    ...
    parser.add_argument("--layout", default=os.environ.get("LAYOUT", "risky_multipath_subgoal"))
    return parser.parse_args(argv)


def main(argv=None):
    smoke_args = parse_args(sys.argv[1:] if argv is None else argv)
    print("bootstrap:")
    for key, value in PATHS.items():
        print(f"  {key}: {value}")
    print(f"layout: {smoke_args.layout}")
    import risky_overcooked_py

    package_path = Path(risky_overcooked_py.__file__).resolve()
    print(f"risky_overcooked_py: {package_path}")
    expected_src = PATHS["risked_src"]
    if expected_src not in package_path.parents:
        raise AssertionError(
            f"Expected risky_overcooked_py from {expected_src}, got {package_path}"
        )
