...
# 이게 reward shaping 부분
# 여기서 rewards 수정하면 hsp reward가 달라짐.
...
# 얘네 전부가 reward shaping에 들어가진 않는다. (에이전트의 성향을 파악하기 위한 CCTV 역할)
EVENT_TYPES = [
    # Tomato events
    "tomato_pickup",         # 토마토 집기
    "useful_tomato_pickup",  # 요리에 필요한 타이밍에 토마토 집기
    "tomato_drop",           # 토마토를 카운터에 내려놓기 (개인 행동)
    "useful_tomato_drop",    # 도움이 되는 위치에 토마토 내려놓기
    "potting_tomato",        # 냄비에 토마토 넣기
    "tomato_risked",         # 토마토를 들고 웅덩이에 들어가기 (위험 감수)
    "tomato_slip",           # 토마토를 들고 웅덩이에서 미끄러짐 (실패)
    "tomato_handoff",        # 파트너가 내려놓은 토마토 이어받기 (협동)

    # Onion events
    "onion_pickup",          # 양파 집기
    "useful_onion_pickup",   # 요리에 필요한 타이밍에 양파 집기
    "onion_drop",            # 양파를 카운터에 내려놓기 (개인 행동)
    "useful_onion_drop",     # 도움이 되는 위치에 양파 내려놓기
    "potting_onion",         # 냄비에 양파 넣기
    "onion_risked",          # 양파를 들고 웅덩이에 들어가기 (위험 감수)
    "onion_slip",            # 양파를 들고 웅덩이에서 미끄러짐 (실패)
    "onion_handoff",         # 파트너가 내려놓은 양파 이어받기 (협동)

    # Dish events
    "dish_pickup",           # 접시 집기
    "useful_dish_pickup",    # 수프가 완성될 즈음 센스있게 접시 집기
    "dish_drop",             # 접시를 카운터에 내려놓기
    "useful_dish_drop",      # 도움이 되는 위치에 접시 내려놓기
    "dish_risked",           # 접시를 들고 웅덩이에 들어가기 (위험 감수)
    "dish_slip",             # 접시를 들고 웅덩이에서 미끄러짐 (실패)
    "dish_handoff",          # 파트너가 내려놓은 접시 이어받기 (협동)

    # Soup events
    "soup_pickup",           # 완성된 수프를 접시에 담기
    "soup_delivery",         # 수프 제출 (최종 목표)
    "soup_drop",             # 수프를 카운터에 내려놓기
    "soup_risked",           # 수프를 들고 웅덩이에 들어가기 (위험 감수)
    "soup_slip",             # 수프를 들고 웅덩이에서 미끄러짐 (실패 - 최악의 상황)
    "soup_handoff",          # 파트너가 내려놓은 수프 이어받기 (협동)

    # Potting events (투입 퀄리티)
    "optimal_onion_potting",       # 레시피에 완벽히 맞는 최적의 양파 투입
    "optimal_tomato_potting",      # 레시피에 완벽히 맞는 최적의 토마토 투입
    "viable_onion_potting",        # 최적은 아니지만 요리에 쓸 수 있는 양파 투입
    "viable_tomato_potting",       # 최적은 아니지만 요리에 쓸 수 있는 토마토 투입
    "catastrophic_onion_potting",  # 레시피를 완전히 망치는 잘못된 양파 투입
    "catastrophic_tomato_potting", # 레시피를 완전히 망치는 잘못된 토마토 투입
    "useless_onion_potting",       # 의미 없는 양파 낭비
    "useless_tomato_potting",      # 의미 없는 토마토 낭비

    "empty_slip",            # 빈 손으로 웅덩이를 지나가다 미끄러짐
    "subgoal_activated",     # 서브골 스위치(G)를 눌러서 웅덩이 비활성화
]
...
class OvercookedGridworld(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._subgoal_timers_activated_this_step = set()
    ...
    def _activate_subgoal(self, subgoal_pos):
        ...
        for water_pos in self.subgoal_to_water[subgoal_pos]:
            ...
            self._subgoal_timers_activated_this_step.add(water_pos)
    ...
    def _tick_subgoal_timers(self):
        ...
        activated_this_step = self._subgoal_timers_activated_this_step
        for water_pos in list(self.water_disable_timers.keys()):
            if water_pos in activated_this_step:
                continue
    ...
    def _reset_dynamic_terrain(self):
        ...
        for water_pos in self.water_disable_timers:
            ...
            self._set_water_active(water_pos, is_active=True)
        self._subgoal_timers_activated_this_step.clear()
    ...
    def resolve_interacts(self, new_state, joint_action, events_infos):
        ...
        for player_idx, (player, action) in enumerate(
            ...
            if current_terrain_type == "G":
                events_infos["subgoal_activated"][player_idx] = True
