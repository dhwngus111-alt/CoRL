...
class TrainerPool:
    ...
    def __init__(self, args, policy_pool: PolicyPool, device = torch.device("cpu")):
        ...
        self.adaptive_milestone_checkpoints = defaultdict(set)
    ...
    def adaptive_checkpoint_interval(self, trainer_name):
        try:
            interval = int(getattr(self.all_args, "adaptive_checkpoint_interval_steps", 0) or 0)
        except (TypeError, ValueError):
            return 0
        if interval <= 0:
            return 0
        try:
            stage = int(getattr(self.all_args, "stage", 0) or 0)
        except (TypeError, ValueError):
            return 0
        if stage != 2:
            return 0
        if str(getattr(self.all_args, "algorithm_name", "")) not in {"adaptive", "mep"}:
            return 0
        if trainer_name != str(getattr(self.all_args, "adaptive_agent_name", "")):
            return 0
        return interval

    def adaptive_checkpoint_dir_name(self):
        dir_name = str(getattr(self.all_args, "adaptive_checkpoint_dir_name", "milestone_checkpoints") or "milestone_checkpoints")
        dir_name = dir_name.strip().strip("/\\")
        return dir_name or "milestone_checkpoints"

    @staticmethod
    def adaptive_milestone_label(milestone_steps):
        milestone_steps = int(milestone_steps)
        if milestone_steps % 1000000 == 0:
            return "{}M".format(milestone_steps // 1000000)
        if milestone_steps % 1000 == 0:
            return "{}K".format(milestone_steps // 1000)
        return str(milestone_steps)

    @staticmethod
    def manifest_has_milestone(manifest_path, milestone_steps):
        if not os.path.exists(manifest_path):
            return False
        try:
            with open(manifest_path, "r", newline="") as f:
                for row in csv.DictReader(f):
                    if int(row.get("milestone_steps", -1)) == int(milestone_steps):
                        return True
    ...
    @staticmethod
    def append_milestone_manifest(manifest_path, row):
        fieldnames = [
            "milestone_steps",
            "actual_trainer_steps",
            "label",
            "model_path",
            "actor_path",
            "critic_path",
    ...
    def save_adaptive_milestone(self, trainer_name, trainer, trainer_step, save_dir, milestone_steps):
        label = self.adaptive_milestone_label(milestone_steps)
        milestone_dir = os.path.join(str(save_dir), trainer_name, self.adaptive_checkpoint_dir_name())
        os.makedirs(milestone_dir, exist_ok=True)
        paths = {"model_path": "", "actor_path": "", "critic_path": ""}

        if self.policy_config(trainer_name)[0].use_single_network:
            model_path = os.path.join(milestone_dir, "model_{}.pt".format(label))
            if not os.path.exists(model_path):
                torch.save(trainer.policy.model.state_dict(), model_path)
            paths["model_path"] = model_path
        else:
            actor_path = os.path.join(milestone_dir, "actor_{}.pt".format(label))
            critic_path = os.path.join(milestone_dir, "critic_{}.pt".format(label))
            if not os.path.exists(actor_path):
                torch.save(trainer.policy.actor.state_dict(), actor_path)
            if not os.path.exists(critic_path):
                torch.save(trainer.policy.critic.state_dict(), critic_path)
            paths["actor_path"] = actor_path
            paths["critic_path"] = critic_path

        manifest_path = os.path.join(milestone_dir, "checkpoint_manifest.csv")
        if not self.manifest_has_milestone(manifest_path, milestone_steps):
            self.append_milestone_manifest(
                manifest_path,
                {
                    "milestone_steps": int(milestone_steps),
                    "actual_trainer_steps": int(trainer_step),
                    "label": label,
        ...
        print("Saved adaptive milestone checkpoint {} {} at step {} to {}".format(
            trainer_name, label, int(trainer_step), milestone_dir
        ))

    def save_adaptive_milestones(self, trainer_name, trainer, trainer_step, save_dir):
        interval = self.adaptive_checkpoint_interval(trainer_name)
        if interval <= 0:
            return
        saved = self.adaptive_milestone_checkpoints[trainer_name]
        milestone_steps = interval
        while milestone_steps <= trainer_step:
            if milestone_steps not in saved:
                self.save_adaptive_milestone(trainer_name, trainer, trainer_step, save_dir, milestone_steps)
                saved.add(milestone_steps)
            milestone_steps += interval
    ...
    def save(self, step, save_dir):
        for trainer_name in self.on_training:
            ...
            self.save_adaptive_milestones(trainer_name, trainer, trainer_step, save_dir)
