"""HSP-style hidden utility schema for Risked Overcooked subgoal layouts.

The active environment exposes raw EVENT_TYPES. HSP S1 hidden utility uses the
original Overcooked category order, with risk/handoff events and transplant
compatibility events appended for subgoal layouts.
"""
...
# Risked env에서 로깅/분석용으로 추가한 event들.
# 기본 HSP S1 preference weight는 원본 many_orders를 따라 0으로 둔다.
...
RISKY_SUBGOAL_EVENT_KEYS = (
    "subgoal_activated",
)

COMPATIBILITY_EVENT_KEYS = RISKY_SUBGOAL_EVENT_KEYS
...
RISKY_RAW_EVENT_KEYS = tuple(EVENT_TYPES)
RISKY_EVENT_KEYS = RISKY_RAW_EVENT_KEYS + tuple(
    key for key in COMPATIBILITY_EVENT_KEYS if key not in RISKY_RAW_EVENT_KEYS
)
RISKY_LOG_ONLY_EVENT_KEYS = (
    "tomato_pickup",
...
RISKY_TO_HSP_CATEGORY = {
    ...
    **{key: (key,) for key in RISKY_MULTIPATH_EVENT_KEYS}, # risked env에서의 추가 event들 매핑
}


def _validate_schema() -> None:
    raw_mapped_keys = set(RISKY_TO_HSP_CATEGORY) - set(COMPATIBILITY_EVENT_KEYS)
    missing_raw = sorted(raw_mapped_keys - set(RISKY_RAW_EVENT_KEYS))
    if missing_raw:
        raise RuntimeError(f"Risked EVENT_TYPES missing mapped raw events: {missing_raw}")

    known_raw = raw_mapped_keys | set(RISKY_LOG_ONLY_EVENT_KEYS)
    unknown_raw = sorted(set(RISKY_RAW_EVENT_KEYS) - known_raw)
    if unknown_raw:
        raise RuntimeError(
            "Risked EVENT_TYPES contains unclassified events. "
            "Decide whether each belongs in hidden utility or logging-only: "
            f"{unknown_raw}"
        )
...
def risky_events_to_hsp_category_info(event_infos: dict) -> list[OrderedDict[str, float]]:
    """Map unambiguous Risky raw events into HSP hidden utility categories.

    Risky raw ``*_pickup`` flags do not encode whether the object came from a
    counter, dispenser, or pot. The adapter fills those source-specific HSP
    categories from transition context.
    """
...
def build_hsp_w0_spec(
    ...
    risked_spec = (
        risked_spec if risked_spec is not None else risky_spec or "0"
    )
    handoff_spec = (
        handoff_spec if handoff_spec is not None else risky_spec or "0"
    )
    subgoal_spec = (
        subgoal_spec if subgoal_spec is not None else risky_spec or "0"
    )
    specs.extend([risked_spec] * len(RISKY_RISKED_EVENT_KEYS))
    specs.extend([handoff_spec] * len(RISKY_HANDOFF_EVENT_KEYS))
    specs.extend([subgoal_spec] * len(RISKY_SUBGOAL_EVENT_KEYS))
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--subgoal-spec", default=None)
    ...
    if args.kind == "w0":
        print(
            build_hsp_w0_spec(
                ...
                subgoal_spec=args.subgoal_spec,
